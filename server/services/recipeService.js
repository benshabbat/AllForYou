...
export const updateRecipe = async (id, recipeData, user) => {
  const { error } = validateRecipe(recipeData);
  if (error) throw new Error(error.details[0].message);
...
export const deleteRecipe = async (id, user) => {
  const recipe = await Recipe.findById(id);
  if (!recipe) return null;
...
// פונקציה לשליפת מתכונים פופולריים
export const getPopularRecipesService = async () => {
  try {
    // שליפת מתכונים עם שדה "popular" מוגדר ל-true
    const recipes = await Recipe.find({ popular: true }).sort({ averageRating: -1 }).limit(10).populate('allergens');
    return recipes;
  } catch (error) {
    console.error('Error in RecipeService.getPopularRecipes:', error.message);
...
export const getPopularRecipes = async (req, res) => {
  try {
    ...
    const recipes = await getPopularRecipesService();
