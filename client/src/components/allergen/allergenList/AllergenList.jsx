...
const AllergenList = ({ allergens = [], showTooltips = true, selectedAllergen, onSelect }) => {
  ...
  const areAllergenIds = allergenArray.length > 0 && typeof allergenArray[0] === 'string';

  const { data: allergenDetails, isLoading, error } = useQuery(
    ['allergens', allergenArray],
    () => areAllergenIds ? api.getAllergensByIds(allergenArray) : Promise.resolve(allergenArray),
    { 
      enabled: allergenArray.length > 0 && areAllergenIds,
  ...
  const displayAllergens = areAllergenIds 
    ? (allergenDetails || []).filter(a => allergenArray.includes(a._id))
    : allergenArray;
      ...
      <ul className={styles.allergenIcons}>
        {displayAllergens.map(allergen => (
          <li 
            ...
            onClick={() => onSelect && onSelect(allergen._id)} 
            style={{ 
              fontWeight: allergen._id === selectedAllergen ? 'bold' : 'normal',
...
export default memo(AllergenList);